...
int redir_reply(struct redir_t *redir, struct redir_socket_t *sock,
  ...
  */
  ...
  if ( stat(appleFile, &appleFileStat) == 0 ) {
      syslog(LOG_DEBUG, "Time Diff :%d", time(NULL) - appleFileStat.st_ctime);
  ...
  while (!done && (redir->starttime + REDIR_HTTP_MAX_TIME) > mainclock_now()) {
	...
	*p2 = 0;
	...
	{ 
	  ...
	  char cmd[128];
	  sprintf(cmd, "/sbin/chilli_query authorize sessiontimeout 180 username wechat_free ip %s ", inet_ntoa(conn->hisip));
