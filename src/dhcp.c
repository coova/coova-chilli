...
int nfqueue_cb_out(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
  ...
  if (ret > 0) {
    ...
    if (ippool_getip(ippool, &ipm, &addr)) {
      ...
      if (_options.debug) 
	syslog(LOG_DEBUG, "dropping packet with unknown destination: %s", inet_ntoa(addr));
...
#define copyres(isq,n)			        \
	...
	if (newlen < 0 || newlen > 1500) {
	  ...
	  struct pkt_iphdr_t  *iph  = pkt_iphdr(pack);
	  struct pkt_udphdr_t *udph = pkt_udphdr(pack);
	  syslog(LOG_DEBUG, "Growing DNS packet from %zu to %d (%ld %d) dt=%d",
...
int dhcp_relay_decaps(struct dhcp_t *this, int idx) {
  ...
  syslog(LOG_DEBUG, "DHCP relay response from %s of length %ld received",
...

  {
    ...
    syslog(LOG_DEBUG, "Sending DHCP relay response %s:%d %ld",
	    inet_ntoa(addr.sin_addr),
...
int dhcp_receive_arp(struct dhcp_ctx *ctx, uint8_t *pack, size_t len) {
  ...
  if (dhcp_hashget(this, &conn, pack_arp->sha)) {
	...
	&& !_options.uamanyip
      ...
      syslog(LOG_DEBUG, "ARP: Unknown client and no dynip: %s", inet_ntoa(taraddr));
      return 0; 
    }
  ...
  if (!reqaddr.s_addr) {
    ...
    syslog(LOG_DEBUG, "ARP: Ignoring self-discovery: %s", inet_ntoa(taraddr));
  ...
  if (!memcmp(&reqaddr.s_addr, &taraddr.s_addr, 4)) { 
    ...
    syslog(LOG_DEBUG, "ARP: Ignoring gratuitous arp %s", inet_ntoa(taraddr));
    return 0;
  }

  /* Is ARP request for clients own address: Ignore */
  if (!memcmp(&conn->hisip.s_addr, &taraddr.s_addr, 4)) {
    syslog(LOG_DEBUG, "ARP: hisip equals target ip: %s", inet_ntoa(conn->hisip));
    return 0;
  }
  ...
  if (!_options.uamanyip) {
    ...
    */
	...
	memcmp(&_options.dhcplisten.s_addr, &taraddr.s_addr, 4)) { 
      ...
      if (_options.debug) {
	syslog(LOG_DEBUG, "ARP: Did not ask for router address: %s", inet_ntoa(conn->ourip));
	syslog(LOG_DEBUG, "ARP: Asked for target: %s", inet_ntoa(taraddr));
      }
    ...
    syslog(LOG_DEBUG, "ARP: Request for %s other than us within our subnet(uamanyip on), ignoring", 
	    inet_ntoa(taraddr));
    return 0;
  }
#endif
